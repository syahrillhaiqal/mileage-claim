...
export default function Approvals({ claims }: ApprovalsProps) {
  // Only show claims that are relevant for approval
  const pendingClaims = claims.filter(c => c.claim_status === 'PENDING' || c.claim_status === 'APPROVED');
    ...
    <div className="flex flex-col h-full space-y-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 shrink-0">
        <div className="flex flex-col">
          ...
          <p className="text-slate-500 mt-1">Review and approve staff mileage claims.</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-6 overflow-y-auto flex-1 pb-8 min-h-0">
        {pendingClaims.map(claim => (
          <div key={claim.claim_id} className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden text-slate-800 shrink-0">
            <div className="p-6 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center text-sm">
              <div className="flex items-center space-x-4">
                <div className="bg-indigo-100 p-2.5 rounded-lg border border-indigo-200">
              ...
              <div className="text-right">
                <div className="text-2xl font-bold tracking-tight text-slate-900">${claim.total_amount.toFixed(2)}</div>
            ...
            <div className="p-6">
              ...
              <div className="space-y-3">
                {claim.trips.map(trip => (
                  <div key={trip.trip_id} className="flex justify-between items-center p-3 rounded-lg border border-slate-100 bg-white text-sm">
                    <div className="flex items-center">
                      <div className="w-1.5 h-1.5 rounded-full bg-slate-300 mr-3"></div>
                      <span className="font-medium">{trip.origin} <span className="text-slate-400 mx-2">→</span> {trip.destination}</span>
                    </div>
                    <div className="flex text-slate-500 space-x-6">
                      <span>{trip.distance} km</span>
                      <span>Toll: ${trip.toll_fee.toFixed(2)}</span>
                      <span className="font-medium text-slate-700 pr-2">${trip.trip_amount.toFixed(2)}</span>
                    </div>
                  </div>
                ))}
              </div>
              ...
              {claim.claim_status === 'PENDING' && (
                <div className="mt-8 flex justify-end space-x-3 pt-6 border-t border-slate-100">
                  <button className="flex items-center px-4 py-2 border border-rose-200 text-rose-700 bg-rose-50 hover:bg-rose-100 rounded-xl text-sm font-medium transition-colors">
                    <X className="w-5 h-5 mr-1" />
                    Reject
                  </button>
                  <button className="flex items-center px-6 py-2 bg-emerald-600 text-white rounded-xl text-sm font-bold hover:bg-emerald-700 shadow-sm transition-colors">
              ...
              {claim.claim_status === 'APPROVED' && (
                <div className="mt-6 pt-4 border-t border-slate-100 flex justify-between items-center text-sm">
                  ...
                  <button className="text-indigo-600 hover:text-indigo-800 font-semibold px-4 py-2 border border-indigo-200 hover:bg-indigo-50 rounded-xl transition-colors">
                    Process Payment
                  </button>
                </div>
              )}
